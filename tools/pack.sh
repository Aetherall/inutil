...
git_sha="${PACK_GIT_SHA:-$(git -C "$REPO_ROOT" rev-parse HEAD 2>/dev/null || printf unknown)}"
git_short="${PACK_GIT_SHORT:-$(git -C "$REPO_ROOT" rev-parse --short HEAD 2>/dev/null || printf unknown)}"
git_describe="${PACK_GIT_DESCRIBE:-$(git -C "$REPO_ROOT" describe --tags --always 2>/dev/null || printf source)}"
version="${1:-${PACK_VERSION:-$git_describe}}"
if [ -n "${PACK_DIRTY:-}" ]; then
  dirty="$PACK_DIRTY"
elif git -C "$REPO_ROOT" rev-parse --git-dir >/dev/null 2>&1; then
  if git -C "$REPO_ROOT" diff --quiet HEAD -- managed native; then dirty=false; else dirty=true; fi
else
  dirty=false
fi
...
# -f net9.0 is REQUIRED, not stylistic: the CLI multi-targets (see its csproj — net6.0 exists for a
# consumer's pre-launch pass), and `publish` on a multi-targeted project with no framework is an error.
dotnet publish "$mg/Inutil.InteropPatch.Cli/Inutil.InteropPatch.Cli.csproj" "${pub_args[@]}" -f net9.0 "${refargs[@]}" -o "$out/tools/inutil-interoppatch"
...
# The SAME patch CLI, published for net6.0 — for a consumer's PRE-LAUNCH pass. Some proxies can only ever be
# patched there: a loader binds them before any in-process patcher exists, and a bound module is unwritable for
# the process lifetime, so no in-game pass can reach them at any point. The only runtime available before the
# game starts is the one the loader ships (BepInEx 6 carries a FLAT net6 CoreCLR with no host), which is why
# this target exists and why it is framework-dependent and RID-less: the consumer drives it through
# coreclr_execute_assembly against that runtime, so it needs the IL and nothing else. Same source, same passes,
# same schema hash as the net9 build above — a second implementation is exactly what must not appear here.
dotnet publish "$mg/Inutil.InteropPatch.Cli/Inutil.InteropPatch.Cli.csproj" \
  -c Release -v q --nologo -f net6.0 --self-contained false "${refargs[@]}" -o "$out/tools/inutil-interoppatch-net6"
[ -f "$out/tools/inutil-interoppatch-net6/inutil-interoppatch.dll" ] || { echo "!! net6 interoppatch publish produced no entry dll" >&2; exit 1; }
rm -f "$out/tools/inutil-interoppatch-net6"/*.pdb   # a pre-launch runner ships IL, not symbols
...
cat > "$out/manifest.json" <<EOF
{
  ...
  "tools": {
    ...
    "inutil-interoppatch-net6": { "path": "tools/inutil-interoppatch-net6", "entry": "inutil-interoppatch.dll",   "invoke": "coreclr_execute_assembly(<loader runtime>, inutil-interoppatch.dll, <interopDir>)", "required": false },
...
echo "   tools: inutil-interoppatch inutil-metadata-extract inutil-check inutil-interoppatch-net6 ($(ls -1 "$out/tools/inutil-interoppatch-net6" | wc -l) files, pre-launch)"
