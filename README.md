...
The flake exposes one game-agnostic distribution per loader on `x86_64-linux`:

```sh
nix build .#bepinex
nix build .#melonloader
```

Each result contains its loader deployment tree, the offline tools, `manifest.json`, and `MARKER`. Copy
`result/bepinex/` or `result/melonloader/` according to the paths recorded in the manifest. The default package
is the BepInEx variant.
