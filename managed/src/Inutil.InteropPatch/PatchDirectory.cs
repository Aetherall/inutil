...
public sealed record DirectoryPatchResult(
    ...
    IReadOnlyList<(string Dll, string Why)> Refused)             // patched in memory, REFUSED by the filesystem — see below
{
    ...
    // Did this walk reach the schema on disk? False when ANY module was refused — the marker is withheld in that
    // case, so a caller can say "partial, next run finishes it" instead of "done".
...
{
    // `detail` governs the PER-MEMBER lines only — every flip, every defer, every residual hole. A full patch of a
    // real game emits ~100k of them, which is the right thing at a maintainer's terminal and the wrong thing when
    // the reader is someone watching a launch (or a log file that has to live with the result). Off, the log still
    // carries one line per assembly plus every total and every refusal, so nothing a caller must ACT on is behind
    // the flag. Structured detail is on the result either way — this only chooses what gets narrated.
    public static DirectoryPatchResult PatchDirectory(string interopDir, TextWriter? log = null, bool detail = true)
    {
        ...
        foreach (string path in Directory.EnumerateFiles(interopDir, "*.dll").OrderBy(p => p, StringComparer.Ordinal))
        {
            ...
            try
            {
                ...
                {
                    // A module the filesystem will not let us REPLACE is not a patch failure — it is a module this
                    // process does not own right now. On the boot that GENERATES the proxies at least one of them is
                    // already mapped into this very process, and a mapped module is unwritable for the process
                    // lifetime (measured: 8 rename attempts with GC + WaitForPendingFinalizers over ~9s, all refused).
                    // Aborting the whole directory on it — which is what an escaping throw did — threw away the ~90
                    // modules that WERE writable and left the caller nothing but a message. Record it, keep going, and
                    // let the marker gate below state the honest verdict: this directory did NOT reach the schema.
                    try
                    {
                    ...
                    catch (Exception ex) when (ex is UnauthorizedAccessException or IOException)
                    {
                        ...
                        log?.WriteLine($">> !! {name}: NOT written — {ex.Message}");
                        continue;
                    }
                    if (result.Flipped > 0)
                    {
                        ...
                        if (detail) foreach (string f in result.Flips) log?.WriteLine($"     FLIP  {f}");
                    }
        ...
        //
        // BOTH are WITHHELD when any module was refused. The marker's whole job is to let a later run skip the work,
        // and the map's is to describe what is on disk — so stamping either after a partial write would tell the next
        // run "this tree is at schema X" about a tree that is not, and the refused module would stay raw forever
        // behind a marker that says otherwise. Withholding is what makes the next launch finish the job.
        string schemaHash = SchemaMarker.Hash(Families.Default());
        if (refused.Count > 0)
        {
            log?.WriteLine($"\n>> !! {refused.Count} module(s) could not be written — marker and exact-type map WITHHELD; " +
                           "this directory is NOT at the schema. Re-run once nothing holds them open.");
            foreach ((string dll, string why) in refused) log?.WriteLine($"   REFUSED  {dll}: {why}");
            // Leave the state IN THE TREE rather than only in a log line nobody will still have. Two later readers
            // need it and neither can derive it: the loader shim's warning (which would otherwise call a 139-of-140
            // tree "not patched") and the next in-process run (which would otherwise re-walk every module to
            // rediscover the one it cannot write). `Converged` is this walk's own verdict on whether another walk
            // could find anything — it flipped nothing, so it could not. See PartialPatchNote.
            try
            {
                new PartialPatchNote(schemaHash, patched.Count, total == 0,
                                     PartialPatchNote.Compute(interopDir), refused).Write(interopDir);
            }
            catch { /* the note is a courtesy; a directory that refuses it still has the withheld marker */ }
        }
        else
        {
            PartialPatchNote.Remove(interopDir);   // completed: the note is now a lie
        ...
        return new DirectoryPatchResult(patched, unchanged, unreadable, total, schemaHash, residual, defers, refused);
    }
    ...
    static void AtomicWrite(ModuleDefinition module, string targetPath)
    {
        ...
        try
        {
            ...
            module.Write(tmp);
            MoveOverReporting(tmp, targetPath);
        }
        finally
        {
    ...
    // Rename-over, reported with the file it was refused on.
    //
    // `File.Move` throws UnauthorizedAccessException with the bare text "Access to the path is denied." and
    // NO path, which makes a refusal on one module indistinguishable from a refusal on any other — the
    // caller logs a sentence that cannot be acted on. The name is the whole diagnosis here: which module is
    // still held tells you WHO is holding it.
    //
    // NOT RETRIED, and that is a measured decision rather than an omission. On the boot that GENERATES the
    // proxies this refusal is permanent for the process lifetime: 8 attempts with GC + WaitForPendingFinalizers
    // over ~9s were all refused (wine/GE-Proton, first-ever boot of a fresh install), which rules out a
    // lingering FileStream and points at a module the generator has MAPPED. Nothing this side can do releases
    // that, so a retry only makes a failing boot slower. The caller catches this per-module and keeps walking.
    static void MoveOverReporting(string tmp, string targetPath)
    {
        try
        {
        ...
        catch (UnauthorizedAccessException ex)
        {
            throw new UnauthorizedAccessException(
                $"rename-over refused for {Path.GetFileName(targetPath)}: {ex.Message}", ex);
        }
        catch (IOException ex)
        {
            throw new IOException($"rename-over failed for {Path.GetFileName(targetPath)}: {ex.Message}", ex);
        }
    }
}
