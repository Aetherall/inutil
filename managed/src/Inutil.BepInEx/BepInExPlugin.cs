...
public sealed class BepInExPlugin : BasePlugin
{
    ...
    private void PersistInteropPatch()
    {
        ...
        // 'mapped' = the preloader found proxies already bound into this process. Nothing this process does can
        // replace a mapped module, so re-walking the whole directory here would spend ~25s to be refused on exactly
        // those again. The next launch has them unbound at ctor time and finishes the job.
        if (verdict is not null && verdict.StartsWith("mapped", StringComparison.Ordinal))
        {
            Log.LogInfo($"inutil: skipping the disk persist — {verdict.Substring("mapped".Length).Trim()} proxy " +
                        "assembl(ies) are bound into this process (normal on the boot that generated them). " +
                        "The next launch patches them on disk.");
            return;
        }
        ...
        if (r.Complete)
            ...
            Log.LogWarning($"inutil: the disk persist reached {r.Patched.Count} assembl{(r.Patched.Count == 1 ? "y" : "ies")} " +
                           $"but {r.Refused.Count} could not be replaced ({string.Join(", ", r.Refused.Select(x => x.Dll))}) — " +
                           "the schema marker is withheld, so the next launch completes them on disk.");
