...
try
{
    ...
    // ── A module the filesystem REFUSES: the walk survives it, and the marker never lies about it ──
    //
    // In the game this is the boot that GENERATES the proxies: the loader has one of its own outputs mapped into
    // the process, and a mapped module cannot be replaced for the process lifetime. An escaping write failure used
    // to abandon the walk mid-directory, which threw away every module after it AND left the ones before it written
    // — a partially-patched tree reported as a total failure. What must hold is the pair below: keep patching, and
    // do NOT stamp the schema marker, because the tree did not reach the schema.
    //
    // The refusal is synthesized portably by parking a DIRECTORY where AtomicWrite wants its temp file, so opening
    // it for write is denied on every platform. It is deliberately ONE module of two: a refusal that takes the whole
    // directory down with it would satisfy "did not stamp" while failing the half the invariant is really about.
    {
        string expectedHash = SchemaMarker.Hash(Families.Default());
        string dir = Path.Combine(work, "interop-refused");
        Directory.CreateDirectory(dir);
        File.Copy(Path.Combine(interopDir, "Assembly-CSharp.dll"), Path.Combine(dir, "Assembly-CSharp.dll"), true);
        File.Copy(Path.Combine(interopDir, "ToyGame.Core.dll"), Path.Combine(dir, "ToyGame.Core.dll"), true);
        Directory.CreateDirectory(Path.Combine(dir, "Assembly-CSharp.dll.inutil-tmp"));   // the write lands here

        DirectoryPatchResult? rr = null;
        Exception? escaped = null;
        try { rr = InteropPatcher.PatchDirectory(dir); } catch (Exception ex) { escaped = ex; }

        Check("an unwritable module does not abort the directory walk", escaped is null, escaped?.Message ?? "no throw");
        if (rr is not null)
        {
            Check("the refused module is reported by name",
                rr.Refused.Count == 1 && rr.Refused[0].Dll == "Assembly-CSharp.dll",
                $"[{string.Join(", ", rr.Refused.Select(x => x.Dll))}]");
            Check("a refused module makes the result INCOMPLETE", !rr.Complete);
            Check("the OTHER module was still patched (the walk kept going)",
                rr.Patched.Any(p => p.Dll == "ToyGame.Core.dll"),
                $"patched [{string.Join(", ", rr.Patched.Select(p => p.Dll))}]");
            // The one that matters: a marker here would tell the NEXT run "this tree is at schema X" about a tree
            // that is not, and the refused module would stay raw forever behind it. Withholding is what makes the
            // next run redo the work.
            Check("an incomplete patch does NOT stamp the schema marker",
                ContentMarker.Verify(dir, SchemaMarker.InteropMarkerFileName, expectedHash) == MarkerVerdict.Missing);
            Check("an incomplete patch does NOT write the exact-type map",
                !File.Exists(Path.Combine(dir, ExactTypeMap.FileName)));
            Check("an incomplete patch leaves a note NAMING the refused module",
                File.Exists(Path.Combine(dir, SchemaMarker.InteropPartialFileName)) &&
                File.ReadAllText(Path.Combine(dir, SchemaMarker.InteropPartialFileName)).Contains("Assembly-CSharp.dll"));

            // ── the note's SKIP decision. It is the one thing here that can cost correctness rather than time: a
            // wrong "yes" silently leaves proxies unpatched, and a mod then meets a raw il2cpp spelling with no
            // warning anywhere. So each condition is asserted to be individually sufficient to say no.
            PartialPatchNote? n = PartialPatchNote.Read(dir);
            Check("the note round-trips through its own reader", n is not null && n.Refused.Count == 1 &&
                n.Refused[0].Dll == "Assembly-CSharp.dll" && n.Schema == expectedHash, n?.Schema);
            if (n is not null)
            {
                bool Bound(string _) => true;   // stand-in for "the loader has this assembly bound"
                Check("a walk that FLIPPED members is not converged — never skip on it", !n.Converged,
                    "the refusing walk flipped 130 members, so another walk may still find work");
                Check("...so it does not license a skip", !n.NothingLeftToDo(dir, expectedHash, Bound));

                var converged = n with { Converged = true };
                Check("a CONVERGED note over unchanged proxies with the module still bound licenses the skip",
                    converged.NothingLeftToDo(dir, expectedHash, Bound));
                Check("...but NOT under a different schema (this build patches differently)",
                    !converged.NothingLeftToDo(dir, "some-other-schema-hash", Bound));
                Check("...and NOT if the refused module is no longer bound (it can be written now)",
                    !converged.NothingLeftToDo(dir, expectedHash, _ => false));

                // The regeneration guard: a game update rewrites every proxy, and a note that outlived it would
                // skip the walk over a tree that has never been patched at all. Rewriting one file is enough to
                // move the fingerprint, because the fingerprint is what stands in for "these same proxies".
                File.SetLastWriteTimeUtc(Path.Combine(dir, "ToyGame.Core.dll"), new DateTime(2030, 1, 1, 0, 0, 0, DateTimeKind.Utc));
                Check("...and NOT after the proxies change underneath it (regeneration guard)",
                    !converged.NothingLeftToDo(dir, expectedHash, Bound),
                    $"note {converged.Fingerprint} vs now {PartialPatchNote.Compute(dir)}");
            }
        }

        // ...and the note must not outlive the condition. Free the write and re-run: the same directory now
        // completes, so the marker appears and the note goes. A note that survived a completing run would keep
        // telling the loader shim "one module is raw" about a tree that is fully patched — the mirror image of the
        // marker lying, and the reason both are written from this one place.
        Directory.Delete(Path.Combine(dir, "Assembly-CSharp.dll.inutil-tmp"));
        DirectoryPatchResult done = InteropPatcher.PatchDirectory(dir);
        Check("re-running after the block clears completes the patch", done.Complete,
            $"still refused [{string.Join(", ", done.Refused.Select(x => x.Dll))}]");
        Check("...and it patches the module that was refused before",
            done.Patched.Any(p => p.Dll == "Assembly-CSharp.dll"),
            $"patched [{string.Join(", ", done.Patched.Select(p => p.Dll))}]");
        Check("...and the marker is finally stamped",
            ContentMarker.Verify(dir, SchemaMarker.InteropMarkerFileName, expectedHash) == MarkerVerdict.Current);
        Check("...and the partial note is REMOVED (it never outlives the condition)",
            !File.Exists(Path.Combine(dir, SchemaMarker.InteropPartialFileName)));
    }
